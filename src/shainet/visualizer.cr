...
module SHAInet
  class Visualizer
    ...
    alias Arrow = GraphViz::Type::GVArrowType
    ...
    def initialize(@title = "graph")
      ...
      @started = false
      @svg_file = File.join(__DIR__, "..", "..", "public", "svg", "graph.svg")
    end
    ...
    def build_label(values, idx)
      ...
      values[idx] = 0.95 if values[idx] >= 1.0
      temp = "#{values[idx].round(rounding_value)}"
      while temp.size < rounding_value + 2
        temp += '0'
      end
      temp
    end

    def hidden_fillcolor(val)
      case val.round(2)
      when .<= 0.0 ; "deepskyblue1"
      when .>= 1.0 ; "yellow"
      else
        "yellow;#{val}:deepskyblue1"
      end
    end
...
#          val_s = (x + 1).to_s
          @nodes << inp.add_node(x.to_s, label: val_s,
                                 ...
                                 color: "midnightblue",
                                 fontname: FONT,
                                 fillcolor: "orange:firebrick")
        end
      end
    end

    def build_hidden
      @graph.add_subgraph("Hidden", GraphViz.new("cluster_2", type: :subgraph)) do |hide|
        hide[:label] = ""
        hide[:labelloc] = "center"
        hide[:labeljust] = "left"
        hide[:fontcolor] = "grey40"
        hide[:style] = "radial"
        ...
        hidden_count.times do |x|
          hidden_sizes[x].times do |y|
            ...
            @nodes << hide.add_node((offset + y).to_s, label: val_s,
                                    fontcolor: "gray7",
                                    style: "filled",
                                    gradientangle: 33,
                                    color: "midnightblue",
                                    fillcolor: hidden_fillcolor(val),
                                    fontname: FONT)
          end
    ...
    def build_output
      @graph.add_subgraph(@data.output.name, GraphViz.new("cluster_3", type: :subgraph)) do |outp|
        ...
        output_size.times do |x|
          val = @data.output.values[@data.mapping.output[x]]
          ...
          @nodes << outp.add_node((offset + x).to_s, label: val_s,
                                  style: "filled",
                                  color: "midnightblue",
                                  fillcolor: "orange:firebrick",
                                  fontname: FONT)
...
#      @graph[:nodesep] = 1.0
    ...
    def generate
      cmd(format: "svg", outfile: @svg_file)
    end
    ...
    def update
      delete_subgraph("Hidden")
      delete_subgraph("Outputs")
      build_hidden
      build_output
      generate
      Fiber.yield
